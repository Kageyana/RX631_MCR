...
#pragma interrupt Excep_SCI1_TXI12 (vect = VECT_SCI12_TXI12)	// 受信割り込み関数定義
#pragma interrupt Excep_SCI1_RXI12 (vect = VECT_SCI12_RXI12)	// 受信割り込み関数定義
...
void init_SCI12( void )
{
	...
	ICU.IER[IER_SCI12_TXI12].BIT.IEN_SCI1_TXI1 = 1;	// TXI割り込み開始
	ICU.IPR[VECT_SCI12_TXI12].BIT.IPR = 13;		// TXI割り込み許可
	
	// Set MPC
	PORTE.PMR.BIT.B1 = 1;			// Disable PE1: peripheral
	PORTE.PMR.BIT.B2 = 1;			// Disable PE2: peripheral
	MPC.PWPR.BIT.B0WI = 0;			// Release protect
	MPC.PWPR.BIT.PFSWE = 1;
...
{
	if ( SCI12_ack_mode == 0 && SCI12.SISR.BIT.IICACKR == 0 ) {
		// ACK受信モード
		SCI12_ack_mode = 1;	// データ送信モード
		SCI12.TDR = *SCI12_I2cDataArry++;	// 送信データ書き込み
		SCI12_I2cNumData--;
	} else if ( SCI12_ack_mode == 1 && SCI12_I2cNumData > 0 ) {
		// データ送信モード
		SCI12.TDR = *SCI12_I2cDataArry++;	// 送信データ書き込み
		SCI12_I2cNumData--;
	} else if ( SCI12_I2cNumData == 0 ) {
		// ストップコンディション発行
		SCI12.SIMR3.BYTE = 0x54;
		while ( SCI12.SIMR3.BIT.IICSTIF == 0 );	// ストップコンディション発行完了まで待つ
		SCI12.SIMR3.BYTE = 0xf0;		// 端子をハイインピーダンスにする
	}
}
//////////////////////////////////////////////////////////////////////////
